...
| envFromSecrets | list | `[{"name":"{{ .Release.Name }}-auth"},{"name":"{{ .Release.Name }}-postgres"},{"name":"{{ .Release.Name }}-obs"}]` | Environment variables from secrets to inject into the container |
